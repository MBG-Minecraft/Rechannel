...
import com.mrbeastmc.rechannel.audio.AudioRelayServer
...
class Application {
	...
	companion object {
		...
		fun main(args: Array<String>) {
			...
			val relayPort = configuration.getRelayPort()
			if (relayPort > 0) {
				AudioRelayServer.start(relayPort)?.let { RelayMixer.initialize(it) }
			}

			Runtime.getRuntime().addShutdownHook(Thread {
				AudioRelayServer.shutdown()
				instance.shutdown()
			})
		}
